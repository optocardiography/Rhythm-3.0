...
function mapPopUp_callback(~,~)
    ...
    switch get(map_popup,'Value')
        ...
        case 9
            % NADH intensity analysis
            GUI_NADH(map,handles,f);
        case 10
            % VM-Ca delay analysis
            GUI_VmCaDelay(map,handles,f);
    end 
        ...
        if handles.bounds(handles.activeScreenNo) == 0
            for x = 1:a
                if get(ensembleAverage_checkbox, 'Value')
                    ...
                    aptracevals=handles.time(1:handles.activeCamData.maxFrame), ...
                    ...
                    for i_cam_index = 1:4
                        if (handles.allCamData(i_cam_index).isloaded && handles.bounds(i_cam_index) == 1)
                            ...
                            plot(handles.time(1:handles.allCamData(i_cam_index).maxFrame),...
                                ...
                                handles.markerColors(i_marker),'LineWidth',2)
                            handles.time(1:handles.allCamData(i_cam_index).maxFrame),...
                                squeeze(handles.allCamData(i_cam_index).cmosData(M(i_marker,2),M(i_marker,1),:))
                            %hold on
                            movegui(w,'center')
                            set(w,'Visible','on')
