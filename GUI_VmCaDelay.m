...
element_width = 0.75;
create_Vmamap_button  = uicontrol('Parent',activationMapGroup,...
                                       'Style','pushbutton',...
                                       'FontSize',fontSize,...
                                       'String','Vm Act. Map',...
                                       'Units','normalized',...
                                       'Position',[pos_left, pos_bottom, element_width, element_height],...
                                       'Callback',@createVmamap_button_callback);
...
pos_bottom = pos_bottom - element_height;
element_width = 0.75;
create_Caamap_button  = uicontrol('Parent',activationMapGroup,...
                                       'Style','pushbutton',...
                                       'FontSize',fontSize,...
                                       'String','Ca Act. Map',...
                                       'Units','normalized',...
                                       'Position',[pos_left, pos_bottom, element_width, element_height],...
                                       'Callback',@createCaamap_button_callback);
                                   
pos_left = 0;
pos_bottom = pos_bottom - 2*element_height;
element_width = 1;
create_VmCaDelaymap_button  = uicontrol('Parent',activationMapGroup,...
                                       'Style','pushbutton',...
                                       'FontSize',fontSize,...
                                       'String','Vm-Ca Delay',...
                                       'Units','normalized',...
                                       'Position',[pos_left, pos_bottom, element_width, element_height],...
                                       'Callback',@createVmCaDelaymap_button_callback);
%%                               
...
%% Button to create activation map
    ...
    function createVmamap_button_callback(~,~)
        ...
        close(gg)
        handles.VmActTimes = handles.activeCamData.saveData;
        
    end

    function createCaamap_button_callback(~,~)
        global CaActTimes;
        % get the bounds of the activation window
        a_start = str2double(get(starttimeamap_edit,'String'));
        a_end = str2double(get(endtimeamap_edit,'String'));
        ...
        handles.activeCamData.drawMap = 1;
        close(gg)
        handles.CaActTimes = handles.activeCamData.saveData;
    end


    function createVmCaDelaymap_button_callback(~,~)
        VmActTimes = handles.VmActTimes;
        CaActTimes = handles.CaActTimes;
        VmCaDelay = VmActTimes - CaActTimes;
        figure();
%         colormap(handles.activeCamData.cmap);
        colormap([1 1 1; jet])
        imagesc(VmCaDelay);
        VmCaDelay_min = min(VmCaDelay(:));
        VmCaDelay_max = max(VmCaDelay(:));
        colorbar('eastoutside','ylim',[VmCaDelay_min VmCaDelay_max])
        
        
         %% Calculating statistics
            VmCaDelay_mean=nanmean(VmCaDelay(:));
            disp(['The average APD in the region is ' num2str(VmCaDelay_mean)])
            VmCaDelay_std=nanstd(VmCaDelay(:));
            disp(['The standard deviation of APDs in the region is ' num2str(VmCaDelay_std)])
            VmCaDelay_median=nanmedian(VmCaDelay(:));
            disp(['The median APD in the region is ' num2str(VmCaDelay_median)])

            handles.activeCamData.meanresults           = sprintf('Mean: %0.3f',VmCaDelay_mean);
            handles.activeCamData.medianresults         = sprintf('Median: %0.3f',VmCaDelay_median);
            handles.activeCamData.SDresults             = sprintf('S.D.: %0.3f',VmCaDelay_std);
